...
class LoginViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let gesture = UITapGestureRecognizer(target: self, action: #selector(imageView_tapped))
    ...
    @objc private func imageView_tapped(){
    ...
    @objc private func didTapLogin(){
        ...
        FirebaseAuth.Auth.auth()
            .signIn(withEmail: email, password: password) { authResult, authError in
                guard let result = authResult, authError != nil else {
                    print("Login error: \(email)")
                    return
                }
                let user = result.user
                print("Log in success: \(user.email)")
            }
    }
    
    private func alertErrorLogin(){
        let alert = UIAlertController(title: "Oops", message: "Please enter all information", preferredStyle: .alert)
